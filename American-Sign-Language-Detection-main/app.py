...
def main():
    ...
    # Model load #############################################################
    # Wrapper de compatibilidad para mediapipe 0.10.x
    class HandsCompat:
        """Wrapper que emula mp.solutions.Hands usando la nueva API de mediapipe 0.10+"""
        ...
        def process(self, image_rgb):
            ...
            if self._static:
                raw = self._detector.detect(mp_img)
            else:
                self._frame_ts += 33
                raw = self._detector.detect_for_video(mp_img, self._frame_ts)
            return _wrap_results(raw)
    ...
    class _WrappedResults:
        def __init__(self, multi_hand_landmarks, multi_handedness):
            self.multi_hand_landmarks = multi_hand_landmarks if multi_hand_landmarks else None
            self.multi_handedness = multi_handedness if multi_handedness else None
    ...
    def _wrap_results(raw):
        ...
        for lm_list, handedness_list in zip(raw.hand_landmarks, raw.handedness):
            fake_lms = [_FakeLandmark(lm.x, lm.y, lm.z) for lm in lm_list]
            mhl.append(_FakeHandLandmarks(fake_lms))
            label = handedness_list[0].category_name
            mhd.append(_FakeHandedness(label))
